 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "92058 92064 92121 91948 92139 92155\n",
    "92067 92065 92122 91962 92182 92173\n",
    "92075 92066 92123 91963\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "        try:\n",
    "            x = pd.DataFrame([[y.replace(\"\\\"\", \"\") for y in x.split(\"\\\",\\\"\")] for x in soup.select(\"pre\")[0].text.split(\"\\n\")])\n",
    "            x.columns = x.iloc[0]\n",
    "            x = x.iloc[1:-1].drop(columns=['View'])  # if error, use drop(columns=['View'], errors='ignore')\n",
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "# Originally you had a commented-out CSV export line here:\n",
    "# overall.to_csv(\"raw_sd_business_data.csv\")"
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [
    {
     "data": {
      "text/plain": [
       "0\n",
       "Name of Firm                                                                 ANGELA EASTVOLD PHD ABPP LLC\n",
       ...
       "Ownership/Self-Certs (Minority, Veteran, Woman, etc)    Self-Certified Small Disadvantaged Business, W...\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "# -----------------------------------------------------\n",
    "# 1. Create a 'FullAddress' column\n",
    "#    Adjust field names if they differ (e.g. 'Address, line 2', 'City', 'State', 'Zip')\n",
    "\n",
    "overall['FullAddress'] = (\n",
    ...
    ")\n",
    "\n",
    "# 2. Create a 'ZipOnly' column (get just the first 5 digits)\n",
    "overall['ZipOnly'] = overall['Zip'].str.extract(r'(\\d{5})')\n",
    "\n",
    "# 3. Save your final data (including new columns) to CSV\n",
    "#    Use index=False to avoid including an extra index column in the CSV\n",
